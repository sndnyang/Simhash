...
public class SimHash {
	...
	public SimHash(String tokens) {
		this.tokens = tokens;this.intSimHash = this.simHash();
	}
	...
	private BigInteger hash(String source) {
		if (source == null || source.length() == 0) {
			...
			BigInteger mask = new BigInteger("2").pow(this.hashbits).
	...
	*/
	public int hammingDistance(SimHash other) {
		BigInteger x = this.intSimHash.xor(other.intSimHash);int tot = 0;
		//统计x中二进制位数为1的个数
		//我们想想，一个二进制数减去1，那么，从最后那个1（包括那个1）后面的数字全都反了，
		//对吧，然后，n&(n-1)就相当于把后面的数字清0，
		//我们看n能做多少次这样的操作就OK了。
		while (x.signum() != 0) {
			tot += 1;x = x.and(x.subtract(new BigInteger("1")));
		}
		return tot;
	}
	...
	public static void main(String[] args) {
		...
		s = "This is a test string for testing, " +
				"This is a test string for testing abcdef";
